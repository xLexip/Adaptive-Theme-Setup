	...
	steps: {
		preparation: {
			...
			intro: {
				...
				explanation:
					`Androidでは、アプリがダークモードなどのシステム設定を自動的に切り替えることが制限されています。これを許可するには、このWebサイトなどを利用してアプリに一回限りの権限 WRITE_SECURE_SETTINGS を付与する必要があります。これは完全に安全であり、アプリがダークモードなどのシステム設定を変更することのみを許可します。アプリをアンインストールすると、このプロセスは完全に元に戻ります。さらに、${appName} とこのセットアップWebサイトの両方が github.com/xLexip でオープンソースとして公開されています。`,
		...
		connection: {
			...
			body: {
				...
				line2:
					'デバイスを選択すると、接続を確認するためのポップアップがAndroidデバイスに表示されます。ポップアップはデバイスのロックが解除されている場合にのみ表示されることがあります。',
			},
			cantFind: {
				title: "デバイスが見つかりませんか？",
				body:
			...
			alreadyInUse: {
				title: "デバイスがすでに使用中ですか？",
				body:
					'他のプログラムがデバイスに自動的に接続したり、操作したりしていないか確認してください。' +
					'ADBがインストールされている場合は、次のコマンドをコンピュータで実行して既存のADB接続を終了できます：',
